...
#[tauri::command]
pub fn generate_spreadsheet() -> i32 {
    let result = xlsx();

    match result {
        Ok(_) => {
            println!("Spreadsheet generated successfully.");
            return 0;
        }
        Err(e) => {
            println!("Error generating spreadsheet: {}", e);
            return 1;
        }
    }
}

fn xlsx() -> Result<(), XlsxError> {
    ...
    workbook.save("C:\\Users\\bashleigh\\Videos\\Form.xlsx")?;
